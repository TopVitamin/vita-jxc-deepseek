...
const ROLE_STATUSES = ['enabled', 'disabled']
...
export default function RolePage() {
  ...
  const columns: Column<SystemRole>[] = [
    ...
    { key: 'name', title: '角色名称', width: '140px' },
    { key: 'description', title: '角色说明', width: '200px' },
    ...
    { key: 'createdAt', title: '创建时间', width: '150px', render: v => formatDateTime(v) },
    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-1">
          ...
          <button onClick={() => alert('权限分配功能：为角色「' + record.name + '」配置菜单和操作权限')} className="px-2 py-1 text-xs text-gray-600 hover:text-primary-600 hover:bg-gray-50 rounded">分配权限</button>
  ...
  return (
    <div>
      <PageHeader title="角色管理" description="管理系统角色，为角色分配菜单和操作权限，控制不同角色的系统功能访问范围">
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500 whitespace-nowrap">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="编码/名称" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500 whitespace-nowrap">状态</label><select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm w-24 focus:outline-none focus:ring-1 focus:ring-primary-500"><option value="">全部</option>{ROLE_STATUSES.map(s => <option key={s} value={s}>{s === 'enabled' ? '启用' : '禁用'}</option>)}</select></div>
      </SearchPanel>
      <DataTable columns={columns} data={data} rowKey="id" loading={loading} pagination={{ current: page, pageSize, total, onChange: setPage }} />

      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingItem ? '编辑角色' : '新增角色'} width="max-w-xl"
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-xs font-medium text-gray-700 mb-1">角色编码 *</label><input {...register('code', { required: true })} placeholder="如：SALES_ADMIN" className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">角色名称 *</label><input {...register('name', { required: true })} placeholder="如：销售管理员" className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
            </div>
          </div>
          <div>
            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">权限说明</h4>
            <div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">角色说明</label><textarea {...register('description')} rows={3} placeholder="描述该角色的职责和权限范围" className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500 resize-none" /></div>
            </div>
          </div>
        </div>
      </FormDialog>

      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="角色详情" width="w-[640px]">
        {viewingItem && (
            ...
            <div>
              ...
              <div className="bg-gray-50/50 rounded-lg p-4">
                <div className="grid grid-cols-2 gap-3">
                  <div><span className="text-xs text-gray-500">角色编码</span><p className="text-sm text-gray-900 mt-0.5 font-medium">{viewingItem.code}</p></div>
                  <div><span className="text-xs text-gray-500">角色名称</span><p className="text-sm text-gray-900 mt-0.5 font-medium">{viewingItem.name}</p></div>
                  ...
                  <div className="col-span-2"><span className="text-xs text-gray-500">角色说明</span><p className="text-sm text-gray-900 mt-0.5">{viewingItem.description || '-'}</p></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <ConfirmDialog open={confirmOpen} title="删除确认" message="确定要删除该角色吗？删除后关联该角色的用户将失去对应权限，此操作不可恢复。" danger confirmText="删除" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
    </div>
  )
}
