...
export default function DictionaryPage() {
  ...
  const columns: Column<SystemDictionary>[] = [
    { key: 'dictCode', title: '字典编码', width: '160px' },
    { key: 'dictName', title: '字典名称' },
    { key: 'itemCode', title: '字典项编码', width: '140px' },
    { key: 'itemName', title: '字典项名称' },
  ...
  const fields = ['dictCode', 'dictName', 'itemCode', 'itemName', 'sort', 'remark']
  const labels: Record<string, string> = { dictCode: '字典编码*', dictName: '字典名称*', itemCode: '字典项编码', itemName: '字典项名称*', sort: '排序', remark: '备注' }
  ...
  return (
    <div>
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="字典编码/名称" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingItem ? '编辑字典' : '新增字典'}
        ...
        <div className="grid grid-cols-2 gap-4">
          {fields.map(key => (
            <div key={key} className={key === 'remark' ? 'col-span-2' : ''}>
              <label className="block text-xs font-medium text-gray-700 mb-1">{labels[key]}</label>
              {key === 'remark' ? (
                <textarea {...register(key)} rows={2} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
              ) : (
                <input {...register(key, { required: labels[key].includes('*') })} type={key === 'sort' ? 'number' : 'text'}
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="字典详情">
        {viewingItem && (
          <div className="space-y-4">
            {[['字典编码', viewingItem.dictCode], ['字典名称', viewingItem.dictName], ['字典项编码', viewingItem.itemCode], ['字典项名称', viewingItem.itemName], ['排序', viewingItem.sort], ['状态', <StatusBadge key="s" status={viewingItem.status} />], ['备注', viewingItem.remark]].map(([label, val]) => (<div key={label as string}><span className="text-xs text-gray-500">{label}</span><p className="text-sm text-gray-900 mt-0.5">{val}</p></div>))}
          </div>
        )}
      </DetailDrawer>

      <ConfirmDialog open={confirmOpen} title="删除确认" message="确定要删除该字典项吗？此操作不可恢复。" danger confirmText="删除" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
    </div>
  )
}
