...
export default function PurchaseOrderPage() {
  ...
  const columns: Column<PurchaseOrder>[] = [
    ...
    { key: 'org', title: '采购组织', width: '100px' },
    { key: 'orderDate', title: '采购日期', width: '100px', render: v => formatDateTime(v) },
    { key: 'expectedDate', title: '预计到货', width: '100px' },
  ...
  const openCreate = () => {
    setEditingItem(null)
    reset({
      orderNo: '', supplierName: '', org: '', orderDate: '', expectedDate: '',
  ...
  const recalcAmount = (idx: number) => {
    const qty = watch(`items.${idx}.quantity`) || 0
    const prc = watch(`items.${idx}.price`) || 0
    return qty * prc
  }
  ...
  const changeStatus = (item: PurchaseOrder, newStatus: string) => {
    handleUpdate(item.id, { docStatus: newStatus, auditStatus: newStatus === 'audited' ? 'audited' : item.auditStatus })
  }
  const onDelete = () => { handleDelete(deletingId); setConfirmOpen(false) }

  return (
    <div>
      <PageHeader title="采购订单" description="管理采购订单，支持多商品明细">
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="订单号/供应商" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingItem ? '编辑采购订单' : '新增采购订单'} width="max-w-3xl"
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div><label className="block text-xs font-medium text-gray-700 mb-1">供应商 *</label><input {...register('supplierName', { required: true })} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
            <div><label className="block text-xs font-medium text-gray-700 mb-1">采购组织</label><input {...register('org')} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
            <div><label className="block text-xs font-medium text-gray-700 mb-1">采购日期</label><input type="date" {...register('orderDate')} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
            <div><label className="block text-xs font-medium text-gray-700 mb-1">预计到货</label><input type="date" {...register('expectedDate')} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={() => append({ id: generateId(), productCode: '', productName: '', unit: '', quantity: 1, price: 0, amount: 0, remark: '' })}
                className="flex items-center gap-1 px-2 py-1 text-xs text-primary-600 border border-primary-300 rounded hover:bg-primary-50">
                <Plus size={12} />添加行
              </button>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">商品编码</th>
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">商品名称</th>
                    ...
                    <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 w-24">单价</th>
                    <th className="px-3 py-2 text-right text-xs font-medium text-gray-500 w-24">金额</th>
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 w-20">备注</th>
                  ...
                  {fields.map((field, idx) => (
                    <tr key={field.id} className="border-b border-gray-100">
                      ...
                      <td className="px-3 py-2 text-center">
                        <button type="button" onClick={() => remove(idx)} className="p-1 rounded hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={14} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="采购订单详情" width="w-[640px]">
        {viewingItem && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              {[['订单号', viewingItem.orderNo], ['供应商', viewingItem.supplierName], ['采购组织', viewingItem.org], ['采购日期', viewingItem.orderDate], ['预计到货', viewingItem.expectedDate], ['状态', <StatusBadge key="s" status={viewingItem.docStatus} />], ['创建人', viewingItem.creator], ['创建时间', formatDateTime(viewingItem.createdAt)]].map(([label, val]) => (
                ...
                <tbody>
                  {viewingItem.items.map(item => (
                    <tr key={item.id} className="border-b border-gray-50">
                      <td className="px-3 py-2 text-sm">{item.productCode}</td><td className="px-3 py-2 text-sm">{item.productName}</td><td className="px-3 py-2 text-sm text-center">{item.unit}</td><td className="px-3 py-2 text-sm text-right">{item.quantity}</td><td className="px-3 py-2 text-sm text-right">{formatCurrency(item.price)}</td><td className="px-3 py-2 text-sm text-right">{formatCurrency(item.amount)}</td>
                    </tr>
                  ))}
                </tbody>
      ...
      <ConfirmDialog open={confirmOpen} title="删除确认" message="确定要删除该采购订单吗？此操作不可恢复。" danger confirmText="删除" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
    </div>
  )
}
