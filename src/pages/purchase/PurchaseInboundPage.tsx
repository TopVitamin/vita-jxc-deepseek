...
export default function PurchaseInboundPage() {
  ...
  const columns: Column<PurchaseInbound>[] = [
    { key: 'inboundNo', title: '入库单号', width: '150px' },
    { key: 'orderNo', title: '来源订单', width: '150px' },
    { key: 'supplierName', title: '供应商' },
    { key: 'warehouseName', title: '入库仓库' },
    { key: 'inboundDate', title: '入库日期', width: '100px', render: v => formatDateTime(v) },
    ...
    { key: 'createdAt', title: '创建时间', width: '100px', render: v => formatDateTime(v) },
    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-1">
          ...
          {record.docStatus === 'pending' && <button onClick={() => handleUpdate(record.id, { docStatus: 'completed' })} className="px-2 py-1 text-xs text-green-600 rounded">审核入库</button>}
  ...
  const onDialogSubmit = (formData: any) => {
    handleCreate({ ...formData, docStatus: 'pending', createdAt: new Date().toISOString() })
    setDialogOpen(false)
  }
  ...
  return (
    <div>
      <PageHeader title="采购入库" description="管理采购入库单">
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="入库单号/订单号" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="新增入库单"
        ...
        <div className="grid grid-cols-2 gap-4">
          {[['inboundNo', '入库单号'], ['orderNo', '来源订单号'], ['supplierName', '供应商*'], ['warehouseName', '入库仓库*'], ['inboundDate', '入库日期'], ['quantity', '入库数量'], ['totalAmount', '入库金额']].map(([key, label]) => (
            <div key={key}>
              <label className="block text-xs font-medium text-gray-700 mb-1">{label}</label>
              <input {...register(key, { required: label.includes('*') })} type={['quantity', 'totalAmount'].includes(key) ? 'number' : key === 'inboundDate' ? 'date' : 'text'}
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="入库单详情">
        {viewingItem && (
          <div className="space-y-4">
            {[['入库单号', viewingItem.inboundNo], ['来源订单', viewingItem.orderNo], ['供应商', viewingItem.supplierName], ['入库仓库', viewingItem.warehouseName], ['入库日期', viewingItem.inboundDate], ['入库数量', viewingItem.quantity], ['入库金额', formatCurrency(viewingItem.totalAmount)], ['状态', <StatusBadge key="s" status={viewingItem.docStatus} />], ['创建时间', formatDateTime(viewingItem.createdAt)]].map(([label, val]) => (
      ...
      <ConfirmDialog open={confirmOpen} title="删除确认" message="确定要删除该入库单吗？" danger confirmText="删除" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
    </div>
  )
}
