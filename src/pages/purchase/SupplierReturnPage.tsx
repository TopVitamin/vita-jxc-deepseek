...
export default function SupplierReturnPage() {
  ...
  return (
    <div>
      <PageHeader title="采购退货" description="管理供应商退货单">
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="退货单号/供应商" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="新增退货单"
        ...
        <div className="grid grid-cols-2 gap-4">
          {[['returnNo', '退货单号'], ['supplierName', '供应商*'], ['warehouseName', '退货仓库*'], ['returnDate', '退货日期'], ['quantity', '退货数量'], ['totalAmount', '退货金额']].map(([key, label]) => (
            <div key={key}>
              <label className="block text-xs font-medium text-gray-700 mb-1">{label}</label>
              <input {...register(key, { required: label.includes('*') })} type={['quantity', 'totalAmount'].includes(key) ? 'number' : key === 'returnDate' ? 'date' : 'text'}
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="退货单详情">
        {viewingItem && (
          <div className="space-y-4">
            {[['退货单号', viewingItem.returnNo], ['供应商', viewingItem.supplierName], ['退货仓库', viewingItem.warehouseName], ['退货日期', viewingItem.returnDate], ['退货数量', viewingItem.quantity], ['退货金额', formatCurrency(viewingItem.totalAmount)], ['状态', <StatusBadge key="s" status={viewingItem.docStatus} />], ['创建时间', formatDateTime(viewingItem.createdAt)]].map(([label, val]) => (
