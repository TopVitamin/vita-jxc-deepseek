...
export default function SalesOrderPage() {
  ...
  const [customerFilter, setCustomerFilter] = useState(filters.customerName || '')
  ...
  const columns: Column<SalesOrder>[] = [
    { key: 'orderNo', title: '销售订单号', width: '160px' },
    { key: 'customerName', title: '客户', width: '180px' },
    { key: 'org', title: '销售组织', width: '110px' },
    { key: 'orderDate', title: '销售日期', width: '110px', render: v => formatDateTime(v) },
    { key: 'expectedDate', title: '预计发货', width: '110px' },
  ...
  const openCreate = () => {
    setEditingItem(null)
    reset({ orderNo: '', customerName: '', org: '', orderDate: '', expectedDate: '', remark: '',
  ...
  const onSearch = () => handleSearch({ keyword, docStatus, customerName: customerFilter, dateFrom, dateTo, dateField: 'orderDate' })
  const onReset = () => { setKeyword(''); setDocStatus(''); setCustomerFilter(''); setDateFrom(''); setDateTo(''); handleReset() }
  ...
  const doStatusAction = (item: SalesOrder, newStatus: string) => { setConfirmAction({ type: 'status', id: item.id, status: newStatus }); setConfirmOpen(true) }
  const onConfirmAction = () => {
    if (confirmAction?.type === 'status') handleUpdate(confirmAction.id, { docStatus: confirmAction.status, auditStatus: confirmAction.status === 'audited' ? 'audited' : confirmAction.status === 'pending_audit' ? 'pending' : confirmAction.status })
  ...
  const getConfirmMessage = () => {
    if (confirmAction?.type === 'status') {
      ...
      return `确定要${labels[confirmAction.status] || '变更状态'}该销售订单吗？`
    }
    return '确定要删除该销售订单吗？此操作不可恢复，相关出库和流水数据可能受到影响。'
  }

  return (
    <div>
      <PageHeader title="销售订单" description="管理销售订单，支持多商品明细行，支持审核、关闭等流程操作">
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500 whitespace-nowrap">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="订单号/客户" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500 whitespace-nowrap">客户</label><input value={customerFilter} onChange={e => setCustomerFilter(e.target.value)} placeholder="客户名称" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-36 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div><label className="block text-xs font-medium text-gray-700 mb-1">客户 *</label><input {...register('customerName', { required: true })} placeholder="请输入客户名称" className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">销售组织</label><select {...register('org')} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500"><option value="">请选择</option>{ORGS.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">销售日期 *</label><input type="date" {...register('orderDate', { required: true })} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">预计发货日期</label><input type="date" {...register('expectedDate')} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2"><h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">商品明细</h4><button type="button" onClick={() => append({ id: generateId(), productCode: '', productName: '', unit: '', quantity: 1, price: 0, amount: 0, remark: '' })} className="flex items-center gap-1 px-2 py-1 text-xs text-primary-600 border border-primary-300 rounded hover:bg-primary-50"><Plus size={12} />添加行</button></div>
            <div className="border rounded-md overflow-hidden">
              <table className="w-full">
                ...
                <tbody>{fields.map((field, idx) => (<tr key={field.id} className="border-b border-gray-100"><td className="px-3 py-2"><input {...register(`items.${idx}.productCode`)} className="w-full px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></td><td className="px-3 py-2"><input {...register(`items.${idx}.productName`)} className="w-full px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></td><td className="px-3 py-2"><input {...register(`items.${idx}.unit`)} className="w-full px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></td><td className="px-3 py-2"><input type="number" {...register(`items.${idx}.quantity`)} className="w-full px-2 py-1.5 border rounded text-sm text-right focus:outline-none focus:ring-1 focus:ring-primary-500" /></td><td className="px-3 py-2"><input type="number" step="0.01" {...register(`items.${idx}.price`)} className="w-full px-2 py-1.5 border rounded text-sm text-right focus:outline-none focus:ring-1 focus:ring-primary-500" /></td><td className="px-3 py-2 text-sm text-right text-gray-700 font-medium">{formatCurrency(recalcAmount(idx))}</td><td className="px-3 py-2"><input {...register(`items.${idx}.remark`)} className="w-full px-2 py-1.5 border rounded text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></td><td className="px-3 py-2 text-center"><button type="button" onClick={() => remove(idx)} className="p-1 rounded hover:bg-red-50 text-gray-400 hover:text-red-500"><Trash2 size={14} /></button></td></tr>))}</tbody>
              </table>
            </div>
          </div>
        </div>
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="销售订单详情" width="w-[680px]">
        {viewingItem && (
          <div className="space-y-6">
            <div>
              ...
              <div className="bg-gray-50/50 rounded-lg p-4">
                <div className="grid grid-cols-2 gap-3">
                  <div><span className="text-xs text-gray-500">销售订单号</span><p className="text-sm text-gray-900 mt-0.5 font-medium">{viewingItem.orderNo}</p></div>
                  <div><span className="text-xs text-gray-500">客户</span><p className="text-sm text-gray-900 mt-0.5 font-medium">{viewingItem.customerName}</p></div>
                  <div><span className="text-xs text-gray-500">销售组织</span><p className="text-sm text-gray-900 mt-0.5">{viewingItem.org || '-'}</p></div>
                  ...
                  <div><span className="text-xs text-gray-500">销售日期</span><p className="text-sm text-gray-900 mt-0.5">{viewingItem.orderDate}</p></div>
                  <div><span className="text-xs text-gray-500">预计发货</span><p className="text-sm text-gray-900 mt-0.5">{viewingItem.expectedDate || '-'}</p></div>
            ...
            <div>
              ...
              <div className="border rounded-md overflow-hidden">
                ...
                <tbody>{viewingItem.items.map(item => (<tr key={item.id} className="border-b border-gray-50"><td className="px-3 py-2 text-sm">{item.productCode}</td><td className="px-3 py-2 text-sm">{item.productName}</td><td className="px-3 py-2 text-sm text-center">{item.unit}</td><td className="px-3 py-2 text-sm text-right">{item.quantity.toLocaleString()}</td><td className="px-3 py-2 text-sm text-right">{formatCurrency(item.price)}</td><td className="px-3 py-2 text-sm text-right font-medium">{formatCurrency(item.amount)}</td><td className="px-3 py-2 text-xs text-gray-500">{item.remark || '-'}</td></tr>))}</tbody>
