...
export default function WarehousePage() {
  ...
  const columns: Column<Warehouse>[] = [
    ...
    { key: 'org', title: '所属组织', width: '110px' },
    { key: 'address', title: '地址' },
    { key: 'manager', title: '负责人', width: '90px' },
  ...
  const fields = [
    ['code', '仓库编码'], ['name', '仓库名称*'], ['type', '仓库类型'], ['org', '所属组织'],
    ['address', '仓库地址'], ['manager', '负责人'], ['phone', '联系电话'],
  ]

  return (
    <div>
      <PageHeader title="仓库管理" description="管理仓库基础信息">
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingItem ? '编辑仓库' : '新增仓库'}
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="仓库详情">
        {viewingItem && (
          <div className="space-y-4">
            {[['仓库编码', viewingItem.code], ['仓库名称', viewingItem.name], ['仓库类型', viewingItem.type], ['所属组织', viewingItem.org], ['仓库地址', viewingItem.address], ['负责人', viewingItem.manager], ['联系电话', viewingItem.phone], ['状态', <StatusBadge key="s" status={viewingItem.status} />], ['创建时间', formatDateTime(viewingItem.createdAt)]].map(([label, val]) => (
      ...
      <ConfirmDialog open={confirmOpen} title="删除确认" message="确定要删除该仓库吗？此操作不可恢复。" danger confirmText="删除" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
    </div>
  )
}
