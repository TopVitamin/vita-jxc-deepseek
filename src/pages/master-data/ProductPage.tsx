...
export default function ProductPage() {
  ...
  const fields = [
    ['code', '商品编码'], ['name', '商品名称*'], ['barcode', '商品条码'], ['category', '商品分类'],
    ['brand', '品牌'], ['unit', '基本单位'], ['purchasePrice', '采购价'], ['salePrice', '销售价'],
  ]

  return (
    <div>
      <PageHeader title="商品管理" description="管理商品基础信息">
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="编码/名称/条码" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingItem ? '编辑商品' : '新增商品'}
        ...
        <div className="grid grid-cols-2 gap-4">
          {fields.map(([key, label]) => (
            <div key={key}>
              ...
              <input {...register(key, { required: label.includes('*') })} type={['purchasePrice', 'salePrice'].includes(key) ? 'number' : 'text'}
      ...
      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="商品详情">
        {viewingItem && (
          <div className="space-y-4">
            {[['商品编码', viewingItem.code], ['商品名称', viewingItem.name], ['商品条码', viewingItem.barcode], ['商品分类', viewingItem.category], ['品牌', viewingItem.brand], ['基本单位', viewingItem.unit], ['采购价', formatCurrency(viewingItem.purchasePrice)], ['销售价', formatCurrency(viewingItem.salePrice)], ['状态', <StatusBadge key="s" status={viewingItem.status} />], ['创建时间', formatDateTime(viewingItem.createdAt)]].map(([label, val]) => (
      ...
      <ConfirmDialog open={confirmOpen} title="删除确认" message="确定要删除该商品吗？此操作不可恢复。" danger confirmText="删除" onConfirm={onDelete} onCancel={() => setConfirmOpen(false)} />
    </div>
  )
}
