...
export default function SupplierPage() {
  ...
  const columns: Column<Supplier>[] = [
    ...
    { key: 'paymentTerms', title: '付款条件', width: '100px' },
  ...
  const openCreate = () => {
    setEditingItem(null)
    reset({ code: '', name: '', category: '', contact: '', phone: '', settlement: '', paymentTerms: '' })
    setDialogOpen(true)
  }
  ...
  return (
    <div>
      <PageHeader title="供应商管理" description="管理供应商基础信息">
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingItem ? '编辑供应商' : '新增供应商'}
        footer={
          <>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input {...register('code')} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">供应商名称 *</label>
            <input {...register('name', { required: true })} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">分类</label>
            <input {...register('category')} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">联系人</label>
            <input {...register('contact')} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">联系电话</label>
            <input {...register('phone')} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">结算方式</label>
            <input {...register('settlement')} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
          </div>
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-700 mb-1">付款条件</label>
            <input {...register('paymentTerms')} className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" />
          </div>
        </div>
      </FormDialog>

      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="供应商详情">
        {viewingItem && (
          <div className="space-y-4">
            {[
              ['供应商编码', viewingItem.code],
              ['供应商名称', viewingItem.name],
              ['分类', viewingItem.category],
              ['联系人', viewingItem.contact],
              ['联系电话', viewingItem.phone],
              ['结算方式', viewingItem.settlement],
              ['付款条件', viewingItem.paymentTerms],
              ['状态', <StatusBadge key="s" status={viewingItem.status} />],
              ['创建时间', formatDateTime(viewingItem.createdAt)],
            ].map(([label, val]) => (
      ...
      <ConfirmDialog open={confirmOpen} title="删除确认" message="确定要删除该供应商吗？此操作不可恢复。" danger confirmText="删除"
