...
export default function StockTransferPage() {
  ...
  const columns: Column<StockTransfer>[] = [
    { key: 'transferNo', title: '调拨单号', width: '160px' },
    { key: 'fromWarehouse', title: '调出仓库', width: '140px' },
    { key: 'toWarehouse', title: '调入仓库', width: '140px' },
    { key: 'transferDate', title: '调拨日期', width: '110px' },
    { key: 'quantity', title: '调拨数量', width: '90px', align: 'right', render: v => v?.toLocaleString() },
  ...
  const openCreate = () => { setEditingItem(null); reset({ transferNo: '', fromWarehouse: '', toWarehouse: '', transferDate: '', quantity: 0, remark: '' }); setDialogOpen(true) }
  ...
  const onSearch = () => handleSearch({ keyword, docStatus, dateFrom, dateTo, dateField: 'transferDate' })
  ...
  const doStatusAction = (item: StockTransfer, newStatus: string) => { setConfirmAction({ type: 'status', id: item.id, status: newStatus }); setConfirmOpen(true) }
  ...
  const getConfirmMessage = () => {
    if (confirmAction?.type === 'status') return confirmAction.status === 'audited' ? '确定要审核通过该调拨单吗？审核后将自动扣减调出仓库库存并增加调入仓库库存。' : '确定要反审核该调拨单吗？'
    return '确定要删除该调拨单吗？此操作不可恢复。'
  }

  return (
    <div>
      <PageHeader title="库存调拨" description="管理仓库间库存调拨，支持调出仓库和调入仓库之间的商品转移">
      ...
      <SearchPanel onSearch={onSearch} onReset={onReset}>
        <div className="flex items-center gap-1.5"><label className="text-xs text-gray-500 whitespace-nowrap">关键词</label><input value={keyword} onChange={e => setKeyword(e.target.value)} placeholder="调拨单号/仓库" className="px-3 py-2 border border-gray-300 rounded-md text-sm w-44 focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
      ...
      <FormDialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editingItem ? '编辑调拨单' : '新增调拨单'} width="max-w-xl"
        ...
        <div className="space-y-4">
          <div>
            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">调拨信息</h4>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-xs font-medium text-gray-700 mb-1">调拨单号</label><input {...register('transferNo')} placeholder="自动生成" className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">调拨日期 *</label><input type="date" {...register('transferDate', { required: true })} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">调出仓库 *</label><select {...register('fromWarehouse', { required: true })} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500"><option value="">请选择</option>{WAREHOUSES.map(w => <option key={w} value={w}>{w}</option>)}</select></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">调入仓库 *</label><select {...register('toWarehouse', { required: true })} className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500"><option value="">请选择</option>{WAREHOUSES.map(w => <option key={w} value={w}>{w}</option>)}</select></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">调拨数量 *</label><input type="number" {...register('quantity', { required: true })} placeholder="0" className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
              <div><label className="block text-xs font-medium text-gray-700 mb-1">备注</label><input {...register('remark')} placeholder="调拨说明" className="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary-500" /></div>
            </div>
          </div>
        </div>
      </FormDialog>

      <DetailDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title="调拨单详情" width="w-[640px]">
        {viewingItem && (
          <div className="space-y-6">
            <div>
              <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">调拨信息</h4>
              <div className="bg-gray-50/50 rounded-lg p-4">
                <div className="grid grid-cols-2 gap-3">
                  <div><span className="text-xs text-gray-500">调拨单号</span><p className="text-sm text-gray-900 mt-0.5 font-medium">{viewingItem.transferNo}</p></div>
                  ...
                  <div><span className="text-xs text-gray-500">调出仓库</span><p className="text-sm text-orange-700 mt-0.5 font-medium">{viewingItem.fromWarehouse}</p></div>
                  <div><span className="text-xs text-gray-500">调入仓库</span><p className="text-sm text-green-700 mt-0.5 font-medium">{viewingItem.toWarehouse}</p></div>
                  <div><span className="text-xs text-gray-500">调拨日期</span><p className="text-sm text-gray-900 mt-0.5">{viewingItem.transferDate}</p></div>
                  <div><span className="text-xs text-gray-500">调拨数量</span><p className="text-sm text-gray-900 mt-0.5 text-lg font-bold">{viewingItem.quantity?.toLocaleString()}</p></div>
